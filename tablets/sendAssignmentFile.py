...
ASSIGNMENT_FILE_PATH = '/Users/64013459//EMCC-2019Server/assignments/BackupAssignments.txt'
...
def validate_file(device_id):
    ...
    tablet_data = subprocess.check_output(
        f'adb -s {device_id} shell cat /mnt/sdcard/Scouting/assignments.txt',
        shell=True)
...
while True:
    ...
    for device in DEVICES:
        if device not in DEVICES_WITH_FILE:
            ...
            subprocess.call(
                f"adb -s {device} push '{ASSIGNMENT_FILE_PATH}' '/mnt/sdcard/Scouting/assignments.txt'",
                shell=True)
